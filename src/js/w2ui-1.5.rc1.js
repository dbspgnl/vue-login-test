...
var w2utils = (function ($) {
    ...
    var obj = {
        ...
        settings : {
            ...
            "dateStartYear"     : 1970,       // start year for date-picker
            "dateEndYear"       : 2040        // end year for date picker
        },
        isBin           : isBin,
        isInt           : isInt,
        isFloat         : isFloat,
...
(function ($) {
    var w2grid = function(options) {
        ...
        this.recordHeight    = 30;       // should be in prototype
    ...
    w2grid.prototype = {
        ...
        buttons: {
            ...
            'add'      : { type: 'button', id: 'w2ui-add', text: '추가', tooltip: '정보추가', icon: 'w2ui-icon-plus' },
            'edit'     : { type: 'button', id: 'w2ui-edit', text: '편집', tooltip: '정보편집', icon: 'w2ui-icon-pencil' },
            'delete'   : { type: 'button', id: 'w2ui-delete', text: '삭제', tooltip: '정보삭제', icon: 'w2ui-icon-cross', disabled: true },
            'save'     : { type: 'button', id: 'w2ui-save', text: '저장', tooltip: '정보저장', icon: 'w2ui-icon-check' }
        },
        ...
        request: function (cmd, add_params, url, callBack) {
           ...
           if (w2utils.settings.dataType == 'HTTP') {
            ...
            if (w2utils.settings.dataType == 'HTTPJSON') {
                 ajaxOptions.data = { jsessionkey:encodeURIComponent(base64session) , request: JSON.stringify(ajaxOptions.data) };
            	// ajaxOptions.data = { jsessionkey:encodeURIComponent(base64session) , request: "" };
          ...
          //  if (this.method) ajaxOptions.type = this.method;
            ...
            // { minp71@daum.net ( park , min ) 
            BASE64EXUTIL.setMap(base64key, base64session);
            var encData = BASE64EXUTIL.encode(ajaxOptions.data.request);
           // ajaxOptions.data.request = encodeURIComponent(encData);
            ajaxOptions.data.request = encData;
            // }
           
           // alert(ajaxOptions.data.request);
           // alert(ajaxOptions.dataType);
           // alert(ajaxOptions.type);
...
/*P.M            setTimeout(function () {
            ...
            // var responseText = this.last.xhr.responseText;
            	var rawtext = BASE64EXUTIL.decode(this.last.xhr.responseText);
        ...
        initColumnOnOff: function () {
            ...
            if (obj.show.saveRestoreState) {
                col_html += '<tr><td colspan="2" onclick="var obj = w2ui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateSave();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('상태반영') + '</div>'+
                            '</td></tr>'+
                            '<tr><td colspan="2" onclick="var obj = w2ui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateReset();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('초기화') + '</div>'+
                            '</td></tr>';
            }
        ...
        message: function(options, callBack) {
            if (typeof options == 'string') {
                options = {
                    ...
                    buttons : '<button class="w2ui-btn" onclick="w2ui[\''+ this.name +'\'].message()">확인</button>',
                    onOpen  : function (event) {
                        setTimeout(function () {
        ...
        message: function(panel, options) {
            ...
            if (typeof options == 'string') {
                options = {
                    ...
                    height  : (options.length < 300 ? 170: 250),
                    body    : '<div class="w2ui-centered">' + options + '</div>',
                    buttons : '<button class="w2ui-btn" onclick="w2ui[\''+ this.name +'\'].message(\''+ panel +'\')">확인</button>',
                    onOpen  : function (event) {
                        setTimeout(function () {
                            $(this.box).find('.w2ui-btn').focus();
                        }, 25);
...
var w2alert = function (msg, title, callBack) {
    var $ = jQuery;
    if (title == null) title = w2utils.lang('알림');
    if ($('#w2ui-popup').length > 0 && w2popup.status != 'closing') {
        w2popup.message({
            width   : 400,
            height  : 170,
            body    : '<div class="w2ui-centered w2ui-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons : '<button onclick="w2popup.message();" class="w2ui-popup-btn w2ui-btn">' + w2utils.lang('확인') + '</button>',
            onOpen: function () {
        ...
        w2popup.open({
            ...
            title     : title,
            body      : '<div class="w2ui-centered w2ui-alert-msg" style="font-size: 13px;">' + msg + '</div>',
            buttons   : '<button onclick="w2popup.close();" class="w2ui-popup-btn w2ui-btn">' + w2utils.lang('확인') + '</button>',
            onOpen: function (event) {
...
var w2prompt = function (label, title, callBack) {
    ...
    var defaults = {
        ...
        value       : '',
        attrs       : '',
        title       : w2utils.lang('알림'),
        ok_text     : w2utils.lang('확인'),
        cancel_text : w2utils.lang('취소'),
        width       : ($('#w2ui-popup').length > 0 ? 400 : 450),
        height      : ($('#w2ui-popup').length > 0 ? 170 : 220),
        callBack    : null
    }
...
(function ($) {
    ...
    w2tabs.prototype = {
        ...
        animateClose: function(id, event) {
        	var remove_flag = confirm("화면을 종료하시겠습니까?");
        ...
        */
        ...
        getHourHTML: function () {
            ...
            for (var a = 0; a < 24; a++) {
                ...
                tmp[Math.floor(a/8)] += '<div class="'+ (this.inRange(tm1) || this.inRange(tm2) ? 'w2ui-time ' : 'w2ui-blocked') + '" hour="'+ leadingZeros(a,2) +'">'+ time +'</div>';
            }

            var html =
                '<div class="w2ui-calendar">'+
                '   <div class="w2ui-calendar-title">'+ w2utils.lang('시간 선택') +'</div>'+
        ...
        getMinHTML: function (hour) {
            ...
            for (var a = 0; a < 60; a += 5) {
                ...
                tmp[ind] += '<div class="'+ (this.inRange(tm) ? 'w2ui-time ' : 'w2ui-blocked') + '" min="'+ leadingZeros(a,2) +'">'+ time +'</div>';
            }
            var html =
                '<div class="w2ui-calendar">'+
                '   <div class="w2ui-calendar-title">'+ w2utils.lang('분 선택') +'</div>'+
                '   <div class="w2ui-calendar-time"><table><tbody><tr>'+
                '       <td>'+ tmp[0] +'</td>' +
                '       <td>'+ tmp[1] +'</td>' +
                '       <td>'+ tmp[2] +'</td>' +
